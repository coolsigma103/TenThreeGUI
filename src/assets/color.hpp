...
struct Color
{
    ...
    {
        return Vector4{color.r / 255.0f, color.g / 255.0f, color.b / 255.0f,
                       color.a};
    }
    ...
    void fromHex(const std::string& hex)
    {
        ...
        if (h.size() == 8)
            a = static_cast<float>(std::stoi(h.substr(6, 2), nullptr, 16)) /
                255.0f;
        else
            a = 1.0f;
    }
    std::string getHexCode(bool includeAlpha = true) const
    {
        ...
        if (includeAlpha)
            snprintf(buffer, sizeof(buffer), "#%02X%02X%02X%02X",
                     ...
                     static_cast<int>(b), static_cast<int>(a * 255.0f));
        else
...
inline void PY_COLOR_HPP(py::class_<Color>& color,
                         py::class_<Color3, Color>& color3)
{
    color
        .def(py::init<float, float, float, float>(), py::arg("r") = 0.0f,
             py::arg("g") = 0.0f, py::arg("b") = 0.0f, py::arg("a") = 1.0f)
    ...
    color3
        .def(py::init<float, float, float>(), py::arg("r") = 0.0f,
             py::arg("g") = 0.0f, py::arg("b") = 0.0f)
        .def(py::init<std::string>(), py::arg("hexCode"))
